...
class ProxyServer:
    ...
    async def start(self) -> None:
        ...
        # If no config is provided, create the gateway directly (native MCP server mode)
        if self.config is None:
            logger.info('Starting in native MCP server mode (no external server)')
            mcp_server = await create_gateway(self.actor_charge_function, self.tool_whitelist)
            app = await self.create_starlette_app(mcp_server)
            await self._run_server(app)
            return

        params: dict = self.config.model_dump(exclude_unset=True)
        ...
        if self.server_type == ServerType.STDIO:
            ...
            async with (
                ...
                # For external stdio servers, this would proxy to them
                # For now, we just create our gateway
                mcp_server = await create_gateway(self.actor_charge_function, self.tool_whitelist)
                app = await self.create_starlette_app(mcp_server)
                await self._run_server(app)
